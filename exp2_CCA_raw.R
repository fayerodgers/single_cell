...
#Run CCA
experiment<- RunMultiCCA(samples, genes.use = genes.use, num.ccs = 30)
pdf(paste0(out,"cc_saturation_raw.pdf"))
p1 <- MetageneBicorPlot(experiment, grouping.var = "infection_status", dims.eval = 1:40, display.progress = FALSE) #use this plot to decide how many CCs to include downstream. Choose 15.
p1
dev.off()
...
for (cell_type in cell_types){
  x = nrow(experimentw@meta.data[which(experiment@meta.data$cell_type==cell_type & experiment@meta.data$infection_status=='CONTROL'),])###HERE
